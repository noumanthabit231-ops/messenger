...
export default function MessagesPage() {
  ...
  const markMessagesAsRead = useCallback(async (senderId: string) => {
    ...
    messages.forEach(msg => { if (msg.sender_id === senderId && !msg.is_read) updateMessageStatus(msg.id, true); });
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        const { error: uploadError } = await supabase.storage.from('voice_messages').upload(fileName, audioBlob, { contentType: 'audio/webm' });
        if (uploadError) { alert('Не удалось отправить голосовое'); } else {
  ...
  const stopRecording = () => { if (mediaRecorder && mediaRecorder.state === 'recording') mediaRecorder.stop(); };
  const handleVoiceButton = () => { if (isRecording) stopRecording(); else startRecording(); };
  const playAudio = (url: string, messageId: string) => {
    if (currentlyPlaying === messageId && audioRef.current && !audioRef.current.paused) { audioRef.current.pause(); setCurrentlyPlaying(null); }
    else {
      if (audioRef.current) { audioRef.current.pause(); audioRef.current.src = url; } else { audioRef.current = new Audio(url); audioRef.current.onended = () => setCurrentlyPlaying(null); }
      audioRef.current.play().catch(e => console.error(e));
      setCurrentlyPlaying(messageId);
    }
  };
  const formatTime = (seconds: number) => `${Math.floor(seconds / 60)}:${(seconds % 60).toString().padStart(2, '0')}`;
  const filteredUsers = realUsers.filter(u => (u.full_name?.toLowerCase() || '').includes(searchQuery.toLowerCase()) || (u.username?.toLowerCase() || '').includes(searchQuery.toLowerCase()));
      ...
      <div className={clsx("w-full md:w-80 border-r border-[#4a5568] flex flex-col bg-[#2d3748] h-full shrink-0", activeChat ? "hidden md:flex" : "flex")}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {filteredUsers.map(realUser => (
            <div key={realUser.id} onClick={() => setActiveChat(realUser)} className={clsx("flex items-center space-x-3 p-4 cursor-pointer hover:bg-[#4a5568] border-b border-[#4a5568]/30", activeChat?.id === realUser.id ? "bg-[#4a5568]" : "")}>
              ...
              <div className="flex-1 min-w-0"><h3 className="text-gray-100 font-medium truncate">{realUser.full_name || realUser.username}</h3></div>
            </div>
          ))}
        </div>
      </div>
            ...
            <div className="h-16 border-b border-[#4a5568] flex items-center justify-between px-3 md:px-6 bg-[#2d3748] shrink-0">
              <div className="flex items-center space-x-2 md:space-x-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className={clsx("text-xs font-medium", activeChat.is_online ? "text-green-400" : "text-gray-400")}>{typingStatus[activeChat.id] ? 'Печатает...' : (activeChat.is_online ? 'В сети' : 'Не в сети')}</p>
                </div>
              </div>
                    ...
                    <div className={clsx("max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-2.5 shadow-sm break-words", isMine ? "bg-blue-600 text-white rounded-br-none" : "bg-[#2d3748] text-gray-100 rounded-bl-none")}>
                      ...
                      {msg.message_type === 'voice' && (
                        <div className="flex items-center gap-3">
                          <button onClick={() => playAudio(msg.file_url!, msg.id)} className="w-10 h-10 bg-blue-700 rounded-full flex items-center justify-center hover:bg-blue-800">{currentlyPlaying === msg.id ? <Pause size={16} className="text-white" /> : <Play size={16} className="text-white ml-0.5" />}</button>
                          <div className="h-1 w-24 bg-[#4a5568] rounded-full overflow-hidden"><div className="h-full w-1/2 bg-blue-400 rounded-full"></div></div>
                        </div>
                      )}
                    </div>
            ...
            <div className="p-3 md:p-4 bg-[#2d3748] border-t border-[#4a5568]">
              <form onSubmit={handleSend} className="flex items-end space-x-2">
                ...
                <div className="flex-1 bg-[#1a202c] border border-[#4a5568] rounded-2xl overflow-hidden">
                  {isRecording ? (
                    <div className="flex items-center space-x-3 px-4 py-3"><div className="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse"></div><span className="text-red-500 font-mono">{formatTime(recordingTime)}</span><button type="button" onClick={stopRecording} className="ml-auto text-red-400 text-xs">Остановить</button></div>
                  ) : (
                ...
                {inputText.trim() ? (
                  ...
                  <button type="button" onClick={handleVoiceButton} className={clsx("p-3 rounded-xl transition-all", isRecording ? "bg-red-500 text-white animate-pulse" : "bg-[#1a202c] border border-[#4a5568] text-gray-400 hover:text-white")}>
