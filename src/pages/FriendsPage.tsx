...
import { Search, UserPlus, UserMinus, Clock, MessageSquare, Check, X } from 'lucide-react';
    ...
    <div className="bg-[#2d3748] border border-[#4a5568] rounded-2xl p-4 flex items-center justify-between hover:border-blue-500/30 transition-colors shadow-sm">
      <div className="flex items-center space-x-3 md:space-x-4 min-w-0 cursor-pointer" onClick={() => p.relStatus === 'friend' && startChat(p)}>
        <div className="relative shrink-0">
          ...
          <div className={clsx("absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-[#2d3748]", (p.is_online && new Date(p.last_seen).getTime() > Date.now() - 2*60*1000) ? "bg-green-500" : "bg-gray-500")}></div>
        </div>
        <div className="min-w-0">
          <h3 className="text-white font-medium text-base md:text-lg truncate">{p.full_name || p.username}</h3>
          <p className="text-gray-400 text-xs truncate">{getLastSeen(p)}</p>
        </div>
      </div>
      <div className="flex space-x-2 shrink-0">
        {p.relStatus === 'friend' && (
          <>
            <button onClick={() => startChat(p)} className="p-2.5 bg-[#1a202c] border border-[#4a5568] hover:bg-blue-600 hover:border-blue-600 text-gray-300 hover:text-white rounded-xl"><MessageSquare size={18} /></button>
            <button onClick={() => handleAction('cancel', p)} className="p-2.5 bg-[#1a202c] border border-[#4a5568] hover:bg-red-600 hover:border-red-600 text-gray-300 hover:text-white rounded-xl"><UserMinus size={18} /></button>
          </>
        )}
        {p.relStatus === 'pending_incoming' && (
          <>
            <button onClick={() => handleAction('accept', p)} className="p-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-xl"><Check size={18} /></button>
            <button onClick={() => handleAction('cancel', p)} className="p-2.5 bg-[#1a202c] border border-[#4a5568] text-gray-400 hover:text-red-500 rounded-xl"><X size={18} /></button>
          </>
        )}
        ...
        {p.relStatus === 'none' && (
          <button onClick={() => handleAction('add', p)} className="p-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-xl"><UserPlus size={18} /></button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto h-full flex flex-col">
      ...
      <div className="relative mb-6 shrink-0">
        ...
        <input type="text" placeholder="Поиск по имени или логину..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-[#2d3748] border border-[#4a5568] rounded-xl pl-12 pr-4 py-3.5 text-white focus:ring-2 focus:ring-blue-500 outline-none shadow-sm" />
      </div>
