...
export default function ProfilePage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto h-full overflow-y-auto no-scrollbar pb-24 md:pb-6">
        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 pb-2 border-b border-[#4a5568]/50">
            <img 
              ...
              alt="Avatar Preview" 
              className="w-16 h-16 rounded-full object-cover bg-[#1a202c] border border-[#4a5568]" 
            />
            <div className="w-full flex-1">
              ...
              <input 
                ...
                className="w-full bg-[#1a202c] border border-[#4a5568] rounded-xl px-3 py-2 text-sm text-white outline-none focus:ring-1 focus:ring-blue-500"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-[#1a202c] border border-[#4a5568] rounded-xl px-3 py-2.5 text-sm text-white outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            <div>
          ...
          <div className="flex items-center justify-between pt-2">
            {isSaved ? (
              <span className="text-green-400 text-xs font-medium flex items-center space-x-1">
                <Check size={16} /> <span>Изменения сохранены!</span>
              </span>
            ) : <div />}
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-colors shadow flex items-center justify-center min-w-[120px]"
            >
      ...
      {/* Блок управления личными постами */}
      <div className="space-y-4">
        ...
        {myPosts.map((post) => (
          <div key={post.id} className="bg-[#2d3748] border border-[#4a5568] rounded-2xl p-4 shadow-sm relative">
            <button 
              ...
              className="absolute top-4 right-4 p-2 text-gray-400 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"
            ...
            {post.image_url && (
              <div className="mb-3 rounded-xl overflow-hidden border border-[#4a5568] max-h-60">
        ...
        {myPosts.length === 0 && (
          <div className="text-center py-8 bg-[#2d3748]/40 border border-dashed border-[#4a5568] rounded-2xl text-gray-500 text-sm">
            Вы еще ничего не публиковали
          </div>
        )}
      </div>
    </div>
