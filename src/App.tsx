...
function App() {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.id) { initBadges(user.id); return cleanup; }
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-[100dvh] bg-[#1a202c] text-white"><div className="animate-spin rounded-full h-12 w-12 border-2 border-t-transparent border-blue-500"></div></div>;
