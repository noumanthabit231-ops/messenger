...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: async (username, password) => {
    ...
    try {
      const { data: authData, error: authError } = await supabase.auth.signInWithPassword({ email: dummyEmail, password });
      if (authError) throw authError;
      if (authData.user) {
        const { data: profile } = await supabase.from('profiles').select('*').eq('id', authData.user.id).single();
        if (!profile) throw new Error('Profile not found');
        await supabase.from('profiles').update({ is_online: true, last_seen: new Date().toISOString() }).eq('id', profile.id);
  ...
  register: async (username, fullName, password) => {
    ...
    try {
      const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
        email: dummyEmail,
        password,
        options: { data: { username: cleanUsername, full_name: fullName.trim(), avatar_url: `https://api.dicebear.com/7.x/bottts/svg?seed=${cleanUsername}` } }
      });
  ...
  updateProfile: async (updates) => {
    ...
    try {
      ...
      alert('Ошибка: возможно, логин занят.');
      set({ isLoading: false });
      return false;
    }
  },
