...
export default function Layout() {
  ...
  const getLastSeen = () => {
    if (!user) return '';
    if (user.is_online && new Date(user.last_seen).getTime() > Date.now() - 2 * 60 * 1000) return 'В сети';
    if (user.last_seen) return `был(а) ${formatDistanceToNow(new Date(user.last_seen), { addSuffix: true, locale: ru })}`;
    return 'Не в сети';
  };
      ...
      <aside className="hidden md:flex w-64 bg-[#2d3748] border-r border-[#4a5568] flex-col shrink-0">
        <div className="p-4 border-b border-[#4a5568] flex items-center space-x-3 shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-400 truncate">{getLastSeen()}</p>
          </div>
        </div>
