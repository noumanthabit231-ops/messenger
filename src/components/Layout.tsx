        ...
        <header className="h-16 border-b border-[#4a5568] bg-[#2d3748] flex items-center justify-between px-4 md:px-6 shrink-0 shadow-sm z-10">
          ...
          <div className="flex items-center space-x-4">
            <button className="text-gray-300 hover:text-white relative p-1.5 rounded-lg active:bg-[#4a5568]">
              <Bell size={22} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 border border-[#2d3748] rounded-full"></span>
            </button>
          </div>
        </header>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center justify-center w-full h-full space-y-1 transition-all ${
                isActive ? 'text-blue-400 scale-105' : 'text-gray-400 active:text-gray-200'
              }`
            }
          >
            <item.icon size={22} />
